const {
  ...
  Hbar,
  PrivateKey,
  ContractFunctionParameters, AccountId, TokenId, Client, ContractId
...
const customerClient = Client.forTestnet().setOperator(AccountId.fromString(process.env.CUSTOMER_ACCOUNT_ID), test_account_private_key);
...
const CallContract = async (
  ...
  const contractTransactionResponse = await (await contractTransaction.signWithOperator(client)).execute(customerClient);

  try {
    const contractReceipt = await contractTransactionResponse.getReceipt(customerClient);
    console.log(contractReceipt.status)
...
const Hashgraph = (client) => ({
  contract: {
    ...
    callCustom: (params) => CallContractCustom(client, params),
